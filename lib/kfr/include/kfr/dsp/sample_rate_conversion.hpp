...
public:
          ...
          input_position(0), output_position(0)
    {
        ...
        // Fix assignment ambiguity by using direct construction
        filter = kfr::univector<T>(size_t(taps), T());
        delay = kfr::univector<T>(size_t(depth), T());
        ...
        // Fix assignment ambiguity by evaluating expression to temporary vector first
        kfr::univector<T> scaled_filter = filter * s;
        for (size_t i = 0; i < filter.size(); ++i) {
            filter[i] = scaled_filter[i];
        }
    }
